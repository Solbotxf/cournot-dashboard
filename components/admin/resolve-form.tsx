...
import { updateAiResult } from "@/lib/admin-api";
...
interface ResolveFormProps {
  marketId: number;
  hasExistingAiResult: boolean;
  porResult: RunSummary | null;
}

export function ResolveForm({ marketId, hasExistingAiResult, porResult }: ResolveFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await updateAiResult(accessCode, marketId, JSON.stringify(aiResultObj));
    ...
    <Card className="border-green-500/30 bg-green-500/5">
      <CardContent className="p-6">
        <p className="text-sm font-medium text-green-400 mb-4 flex items-center gap-2">
          ...
          {hasExistingAiResult ? "Update AI Result" : "Submit AI Result"}
        </p>
        {!hasExistingAiResult && (
          <p className="text-xs text-amber-400 mb-4">
            Note: The backend requires an existing AI result before you can update it.
            If the AI has not processed this market yet, this submission may fail.
          </p>
        )}
